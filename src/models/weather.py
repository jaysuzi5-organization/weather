...
This module defines:
- The SQLAlchemy ORM model for persisting Weather data.
- The Pydantic schema for validating API requests when creating a Weather.
...
class Weather(Base):
    """
    SQLAlchemy ORM model representing a Weather record.
    ...
    def __repr__(self):
        """
        Returns a string representation of the Weather instance.

        Example:
            <Weather(id=1, username='johndoe', email='john@example.com')>
        """
        return f"<Weather(id={self.id}, username='{self.username}', email='{self.email}')>"


class WeatherCreate(BaseModel):
    """
    Pydantic schema for creating a new Weather.
